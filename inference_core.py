...
class TorchImageCache:
    ...
    def __init__(self, image_paths: List[str], divide_by: int = 16,
        ...
        assert len(self) > 0
        self.pad_array = None
        self.channels, self.height, self.width = self[0].shape
...
class MaskCache:
    ...
    @torch.no_grad()
    def get_torch(self, idx: int) -> np.ndarray:
        mask = torch.ByteTensor(self[idx])
        return pad_divide_by(mask, self.divide_by)[0]
...
class TorchProbsCache:
    ...
    def __getitem__(self, idx: int) -> torch.Tensor:
        ...
        if not os.path.exists(prob_path):
            probs = torch.zeros((self._num_obj, self.height, self.width),
                               device=self.dev)
            probs[0] = 1e-7
            return probs
    ...
    def update(self, idx: int, probs: torch.Tensor) -> None:
        ...
        prob_path = self.__prob_path(idx)
        probs = probs.squeeze()
        assert probs.shape[-2:] == (self.height, self.width)
...
class InferenceCore:
    ...
    def get_image_buffered(self, idx):
        ...
        if idx not in self.image_buf:
            ...
            if len(self.image_buf) > self.i_buf_size:
                self.image_buf = {}
            self.image_buf[idx] = self.images[idx].unsqueeze(0).to(self.device)
    ...
    def interact(self, mask, idx, total_cb=None, step_cb=None):
        ...
        self.mask_diff = mask.squeeze() - self.prob[idx].to(self.device)
